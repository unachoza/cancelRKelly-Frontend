...
class UsageStats extends Component {
  ...
  userCounter = async () => {
    const res = await axios.get('https://spotify-cancel.herokuapp.com/db/users/unique');
    this.setState({ userCount: res.data.data[0].count });
  };

  songsRemovedCounter = async () => {
    const res = await axios.get('https://spotify-cancel.herokuapp.com/db/deletedsongs');
